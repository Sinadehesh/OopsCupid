...
const VECTORS = [
  { key: "digital" as const, label: "Digital Behavior", icon: Smartphone, desc: "Phone secrecy, app switching, notification hiding, passcode changes" },
  { key: "chronological" as const, label: "Time & Schedule", icon: Clock, desc: "Unexplained absences, late returns, rehearsed whereabouts" },
  { key: "intimacy" as const, label: "Emotional Distance", icon: Heart, desc: "Withdrawal, coldness, picking fights to create space" },
  { key: "micro" as const, label: "Story Inconsistencies", icon: Search, desc: "Small lies that drift, over-explaining, defensive reactions" },
];
      ...
      <div className={`${cfg.heroBg} text-white rounded-[32px] p-8 md:p-12 mb-8 text-center shadow-2xl relative overflow-hidden border ${cfg.heroBorder}`}>
        ...
        <div className="relative z-10">
          <div className={`inline-flex items-center gap-2 px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest mb-6 border ${cfg.badgeColor}`}>
            <ShieldAlert className="w-4 h-4" /> Infidelity Risk: {data.riskLevel}
          </div>
        ...
        {VECTORS.map(({ key, label, icon: Icon, desc }) => (
          <div key={key} className="bg-white rounded-3xl p-7 border border-slate-200 shadow-sm">
            ...
            <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
              <div className={`${cfg.barColor} h-full rounded-full transition-all duration-1000`} style={{ width: `${data.vectors[key]}%` }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div ref={paywallRef} className="scroll-mt-8">
        <div className="bg-gradient-to-r from-rose-600 via-rose-500 to-pink-600 rounded-3xl p-6 mb-6 text-center shadow-lg">
          <p className="text-white font-black text-lg md:text-xl">
            Your surface scan ends here. The full investigation is below.
          </p>
        </div>
        ...
        <div className="bg-[#0d0a12] rounded-[32px] p-8 md:p-12 shadow-2xl border border-rose-500/20 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-lg mx-auto text-center">
            ...
            <div className="inline-flex items-center gap-2 bg-rose-500/20 border border-rose-500/40 text-rose-300 text-xs font-black uppercase tracking-widest px-5 py-2 rounded-full mb-6">
              <Zap className="w-3.5 h-3.5" /> Full Cheating Investigation Report
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-10 text-left">
              {[
                { icon: "\uD83D\uDD0D", text: "Paranoia or proof? AI verdict" },
                { icon: "\uD83D\uDCF1", text: "What he is doing on his phone" },
                { icon: "\uD83D\uDDD3\uFE0F", text: "His schedule anomalies decoded" },
            ...
            <div className="space-y-2 mb-8">
              {[
                ["Full AI Infidelity Investigation", "$45"],
                ["Cheating Signal Breakdown (12 markers)", "$30"],
                ["Word-for-word Confrontation Script", "$25"],
                ["Digital Behavior Deep-Dive", "$20"],
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 mb-8">
              ...
              <div className="flex items-center justify-center gap-5">
                <div className="text-left">
                  ...
                  <p className="text-[#ffbc42] text-xs font-black uppercase tracking-wider mt-1">One-time only</p>
                </div>
