...
// ... (PRESERVING YOUR EXISTING ARRAYS: demoQ, ecrQ, rsQ, dersQ, loveQ, yqsQ, crqQ, mdsQ, parQ) ...
...
]; // Truncated for brevity in script, but assumes you keep the rest of your battery here in your actual file.
...
export default function QuizWidget({ quizName }: { quizName: string }) {
  ...
  const activeQuestions = useMemo(() => {
    if (quizName === "attachment-style") {
      ...
      return [...demoQ, ...ecrQ]; // Note: Ensure you keep your full arrays here
    }
    
    // INTEGRATING THE NEW 50-QUESTION BATTERY
    if (isAttraction) {
  ...
  }, [quizName, answers["demo_2"], isAttraction]);
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      if (quizName === "attachment-style") {
        ...
        // TRIGGER NEW ATTRACTION SCORING
  ...
  if (showResult && resultData) {
    ...
    // RENDER NEW ATTRACTION REPORT (BUILT IN STEP 4)
