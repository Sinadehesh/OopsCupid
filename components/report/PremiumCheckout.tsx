...
import { ShieldCheck, BrainCircuit, HeartHandshake, Zap, Loader2, Clock, ArrowRight, Lock, BookOpen, Star } from "lucide-react";

interface PremiumCheckoutProps {
  onUnlock: () => void;
  isGenerating?: boolean; // Made optional so it works smoothly with Router redirects
  archetype: string;
  relationshipStatus: string;
}

export default function PremiumCheckout({ onUnlock, isGenerating = false, archetype, relationshipStatus }: PremiumCheckoutProps) {
  ...
  return (
    <div className={`relative w-full max-w-6xl mx-auto bg-white border border-[#d6d2d2] rounded-2xl shadow-md mt-12 overflow-hidden`}>
      <div className="bg-[#f0c808] text-[#086788] text-center py-3 px-4 flex items-center justify-center gap-2 font-black text-sm tracking-wide">
        <Clock className="w-5 h-5" /> AI Server Allocation Secured For The Next 15 Minutes
      </div>
      ...
      <div className="p-6 md:p-12 grid grid-cols-1 lg:grid-cols-5 gap-8 bg-[#fff1d0]/20">
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className={`flex gap-5 p-6 rounded-xl bg-white border border-[#d6d2d2] shadow-sm`}>
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2">
                <h4 className={`text-lg font-black text-[#086788]`}>2. Boyfriend & Partner Analysis</h4>
                <span className="text-sm font-bold text-[#086788]/40 line-through">$97 Value</span>
              </div>
              <p className={`text-base font-medium text-[#086788]/80`}>See the exact type of toxic behaviors you naturally attract, and how your current/past partners manipulate your triggers.</p>
            </div>
          </div>
          ...
          <div className={`flex gap-5 p-6 rounded-xl bg-white border-2 border-[#f0c808] shadow-md relative overflow-hidden`}>
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2">
                <h4 className={`text-lg font-black text-[#086788]`}>3. His Playbook & Master Action Plan</h4>
        ...
        <div className="lg:col-span-2 flex flex-col justify-center">
          <div className={`p-8 md:p-10 rounded-2xl bg-white border border-[#d6d2d2] text-center shadow-md relative`}>
            ...
            <button onClick={onUnlock} disabled={isGenerating} className="w-full min-h-[64px] bg-[#f0c808] text-[#086788] rounded-xl font-black text-xl transition-all shadow-md hover:-translate-y-1 flex items-center justify-center gap-3 disabled:opacity-80 disabled:hover:translate-y-0 cursor-pointer group">
              {isGenerating ? <><Loader2 className="w-6 h-6 animate-spin text-[#086788]" /> Routing...</> : <>Unlock My Playbook <ArrowRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" /></>}
            </button>
          </div>
        </div>
      </div>
      <div className={`p-6 md:p-8 border-t border-[#d6d2d2] bg-white flex flex-col sm:flex-row items-center gap-4 justify-center`}>
        ...
        <p className={`text-sm md:text-base font-bold text-[#086788]/80`}>7-Day "Diary Reader" Guarantee. Not accurate? Full refund. No questions asked.</p>
      </div>
    </div>
  );
}
