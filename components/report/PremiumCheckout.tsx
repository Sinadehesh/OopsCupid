      ...
      <div className={`relative z-10 rounded-3xl border shadow-2xl overflow-hidden max-w-3xl mx-auto ${bgCard}`}>
        ...
        <div className="p-8 md:p-10 space-y-6">
          ...
          <div className={`flex gap-6 p-6 rounded-2xl border ${bgPillar}`}>
            <div className="shrink-0 flex items-center justify-center w-14 h-14 rounded-full bg-[#9d0208]/10 text-[#9d0208]">
          ...
          <div className={`flex gap-6 p-6 rounded-2xl border relative overflow-hidden ${bgPillar}`}>
            <div className="absolute top-0 right-0 bg-[#fca311] text-[#000000] text-[10px] font-black px-3 py-1 uppercase tracking-widest rounded-bl-lg">
              Highest Value
            </div>
            <div className="shrink-0 flex items-center justify-center w-14 h-14 rounded-full bg-[#fca311]/10 text-[#fca311]">
          ...
          <div className={`flex gap-6 p-6 rounded-2xl border ${bgPillar}`}>
            <div className="shrink-0 flex items-center justify-center w-14 h-14 rounded-full bg-[#14213d]/10 text-[#14213d] dark:bg-[#e5e5e5]/10 dark:text-[#e5e5e5]">
        ...
        <div className={`p-8 md:p-10 border-t ${isDarkTheme ? 'bg-[#000000]/50 border-[#ffffff]/10' : 'bg-[#e5e5e5]/50 border-[#000000]/10'}`}>
          ...
          <button 
            ...
            className="w-full py-5 bg-[#9d0208] hover:bg-[#9d0208]/90 text-[#ffffff] rounded-2xl font-extrabold text-xl md:text-2xl transition-all shadow-xl hover:shadow-2xl hover:-translate-y-1 flex items-center justify-center gap-3 disabled:opacity-80 disabled:hover:translate-y-0 cursor-pointer"
          >
            {isGenerating ? (
              <><Loader2 className="w-7 h-7 animate-spin text-[#fca311]" /> Processing Secure Payment...</>
            ) : (
              <><Lock className="w-6 h-6" /> Unlock Everything Now</>
            )}
          </button>

          <div className="mt-6 flex items-center justify-center gap-2 text-[#14213d] dark:text-[#e5e5e5] opacity-80">
            <ShieldCheck className="w-5 h-5 text-[#fca311]" />
            <span className="text-sm font-bold">7-Day Guarantee: 100% money back if it's not accurate.</span>
          </div>
        </div>
