...
const RISK_CONFIG = {
  SEVERE:   { label: "High-Risk: Active Concealment Detected", badge: "bg-rose-100 text-rose-700 border-rose-200",   bar: "from-rose-400 to-rose-600",   ring: "#f43f5e" },
  ELEVATED: { label: "Elevated Risk: Suspicious Pattern",      badge: "bg-amber-100 text-amber-700 border-amber-200",  bar: "from-amber-400 to-amber-600", ring: "#f59e0b" },
  MODERATE: { label: "Moderate Risk: Gray Zone Signals",        badge: "bg-indigo-100 text-indigo-700 border-indigo-200", bar: "from-indigo-400 to-indigo-600", ring: "#6366f1" },
};
...
function AIInsightCard({ icon: Icon, title, subtitle, content, color }: { icon: any; title: string; subtitle: string; content: string; color: string }) {
  ...
  const titleCl = color === "rose" ? "text-rose-700" : "text-slate-900";
  return (
    <div className={`rounded-[28px] p-8 border shadow-sm ${accent}`}>
      <div className="flex items-start gap-5 mb-5">
        ...
        <div>
          <h4 className={`font-black text-xl leading-tight ${titleCl}`}>{title}</h4>
    ...
    <div className="rounded-[28px] p-8 border border-slate-200 bg-white shadow-sm">
      ...
      <div className="space-y-2">
        {["w-full","w-5/6","w-4/5","w-full","w-3/4"].map((w, i) => (
          <div key={i} className={`h-3 bg-slate-100 animate-pulse rounded-lg ${w}`} />
        ))}
      </div>
    </div>
  );
...
export default function InfidelityMasterReport({ data }: { data?: ReportData }) {
  ...
  // ── Safe defaults so nothing below ever crashes ──
  const score      = data?.score ?? 65;
  const riskLevel  = (data?.riskLevel === "SEVERE" || data?.riskLevel === "MODERATE") ? data.riskLevel : "ELEVATED";
  const cfg        = RISK_CONFIG[riskLevel];
  const safeVec    = {
    digital:       data?.vectors?.digital       ?? score,
    chronological: data?.vectors?.chronological ?? score,
    intimacy:      data?.vectors?.intimacy      ?? score,
    micro:         data?.vectors?.micro         ?? score,
  };
  ...
  useEffect(() => {
    if (!data) { setAiLoading(false); return; }
    const generate = async () => {
      try {
        const res  = await fetch("/api/infidelity-ai-report", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ result: { ...data, vectors: safeVec } }),
        });
    ...
    <div className="max-w-4xl mx-auto py-12 px-4 md:px-8 bg-[#f8fafc] font-sans">
        ...
        {VECTORS.map(({ key, label, icon: Icon, desc }) => (
          <div key={key} className="bg-white rounded-3xl p-7 border border-slate-200 shadow-sm">
            ...
            <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
              <div
                className={`bg-gradient-to-r ${cfg.bar} h-full rounded-full transition-all duration-[1500ms] ease-out`}
                style={{ width: mounted ? `${safeVec[key]}%` : "0%" }}
              />
            </div>
          </div>
        ))}
      ...
      {aiLoading && (
        <div className="flex items-center gap-4 bg-white border border-slate-200 rounded-3xl px-7 py-5 mb-8 shadow-sm">
          <div className="w-8 h-8 border-2 border-rose-200 border-t-rose-600 rounded-full animate-spin shrink-0" />
          <div>
      ...
      <div className="space-y-5 mb-14">
        {INSIGHT_SECTIONS.map((s) =>
          insights ? (
            <AIInsightCard key={s.key} icon={s.icon} title={s.title} subtitle={s.subtitle} content={insights[s.key]} color={s.color} />
          ) : (
      ...
      {aiError && (
        <div className="bg-rose-50 border border-rose-200 rounded-3xl p-7 mb-8 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-rose-600 text-sm font-medium">Your scores above are accurate. Reload to retry the AI sections.</p>
          </div>
        </div>
      )}
      ...
      {insights && (
        <div className="bg-[#0d0a12] rounded-[32px] p-10 text-center relative overflow-hidden">
          ...
          <p className="text-white/55 font-medium text-lg mb-8 max-w-lg mx-auto leading-relaxed">
            Screenshot this report. Follow your 5-day protocol. Trust what you found.
          </p>
          <button
            onClick={() => window.print()}
            className="bg-gradient-to-r from-rose-600 to-pink-600 text-white font-black text-lg px-10 py-5 rounded-2xl shadow-xl shadow-rose-500/30 hover:from-rose-500 hover:to-pink-500 transition-all"
          >
