...
echo "🚀 Updating UnlockBanner..."

# Overwrite UnlockBanner.tsx with the new functional Link and strict pricing
cat << 'EOF' > components/report/UnlockBanner.tsx
...
echo "🛠️ Injecting dynamic prop into AttachmentReport.tsx..."
# Use sed to safely find and replace the empty <UnlockBanner /> with the one holding the user's result prop
sed -i 's/<UnlockBanner \/>/<UnlockBanner primaryStyle={profile.attachment.general.classification} \/>/g' components/report/AttachmentReport.tsx

echo "💾 Committing and pushing to GitHub..."
git add components/report/UnlockBanner.tsx components/report/AttachmentReport.tsx
git commit -m "fix: make unlock banner functional with $19.99 strict pricing and dynamic routing"
git push origin feature/premium-bundle-complete

echo "✅ BANNER FIXED AND PUSHED TO VERCEL!"
